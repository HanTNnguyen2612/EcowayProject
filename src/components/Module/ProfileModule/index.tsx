...
export default function ProfilePage() {
  ...
  const profile: ScanHistoryItem[][] = React.useMemo(() => {
    const groupedByDate = data?.historyScan?.reduce<{
      [key: string]: ScanHistoryItem[];
    }>((acc, item) => {
  ...
  return (
    <div className="min-h-screen bg-[#0B3B2D] relative overflow-hidden">
      ...
      {/* Wave Divider */}
      <div
        className="absolute -left-4 -right-4 h-16 rounded-[100px_100px_0_0] bg-white transform -translate-y-10"
        style={{}}
      />

      {/* Scan History Section */}
      <div className="relative bg-white min-h-screen px-6 pt-12">
        <h2 className="text-xl font-bold mb-4 text-[#0B3B2D]">Lịch sử scan</h2>
        <div className="flex flex-col justify-center gap-6">
          {profile?.map((items: ScanHistoryItem[]) => (
            <div key={items[0].updatedAt} className="mt-6 ">
              ...
              {items.map((item) => (
                <div className="space-y-4" key={item.id_scan}>
                  <Card className="p-4 shadow-sm">
                    <div className="bg-white rounded-lg p-4 mb-3 last:mb-0 border border-gray-100">
