...
export function QRScanModal({ isOpen, onClose, isSuccess }: QRScanModalProps) {
  ...
  const isLoggiedIn = webStorageClient.getToken() ? true : false;
      ...
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader
              className={cn(
                "flex justify-center font-semibold text-3xl gap-1 ",
            ...
            <ModalFooter>
              <Button
                color="danger"
                variant={isSuccess ? "light" : "shadow"}
                onPress={() => {
                  router.push("/scan");
                  onClose();
                }}
              ...
              {isSuccess && (
                <Button color="primary" onPress={handleProfileNavigation}>
                  Trang cá nhân
                </Button>
              )}
            </ModalFooter>
